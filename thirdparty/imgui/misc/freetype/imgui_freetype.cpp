...
static bool ImGui_ImplFreeType_FontBakedLoadGlyph(ImFontAtlas* atlas, ImFontConfig* src, ImFontBaked* baked, void* loader_data_for_baked_src, ImWchar codepoint, ImFontGlyph* out_glyph, float* out_advance_x)
{
    ...
    // [Solace] Use the hinter's grid-fitted advance, not slot->linearHoriAdvance.
    //
    // The linear advance is what the browser lays out with, so reporting it made
    // a run measure exactly as wide as its CSS counterpart. But RenderText rounds
    // the pen to a whole pixel for every glyph (imgui_draw.cpp, pen_x), and a pen
    // walking on fractional advances lands on a different subpixel phase each
    // time: the same pair of letters comes out 1px apart here and 3px apart
    // there, and word spaces alternate 5 and 6. Text rendered that way reads as
    // stuck together in places and gappy in others.
    //
    // Grid-fitting the advance keeps the pen on whole pixels the whole way
    // across, which is the entire point of hinting. Runs can now differ from the
    // browser by a fraction of a pixel per glyph - but every width in this app is
    // measured with these same advances, so the layout stays self-consistent.
    const float advance_x = ((float)slot->advance.x / 64.0f) / rasterizer_density;
