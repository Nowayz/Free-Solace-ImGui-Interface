...
// [Solace] Optional pair-kerning hook, installed on ImFontAtlas.
// Dear ImGui has no text shaping, so a run is the plain sum of its advances,
// while browsers apply the font's `kern` feature by default (~2% narrower).
// Set ImFontAtlas::KerningFunc to have every ImGui text path - including
// InputText's layout, caret and hit-testing - apply the same adjustments.
// Return the adjustment in pixels for the pair (c_prev, c) at 'size'.
typedef float (*ImFontKerningFunc)(ImFont* font, float size, unsigned int c_prev, unsigned int c, void* user_data);
...
struct ImFontAtlas
{
    ...
    ImFontKerningFunc           KerningFunc;        // [Solace] Optional pair-kerning hook. NULL to disable.
    void*                       KerningUserData;    // [Solace] Passed through to KerningFunc.
