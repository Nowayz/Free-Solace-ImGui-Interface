...
{
    unsigned int c;
    const char* at = obj->TextSrc + line_start_idx + char_idx;
    const char* text_end = obj->TextSrc + obj->TextLen;
    ImTextCharFromUtf8(&c, at, text_end);
    if ((ImWchar)c == '\n')
        return IMSTB_TEXTEDIT_GETWIDTH_NEWLINE;
    ImGuiContext& g = *obj->Ctx;
    float width = g.FontBaked->GetCharAdvance((ImWchar)c) * g.FontBakedScale;

    // [Solace] Match the kerned layout, so clicking lands on the right character.
    if (char_idx > 0)
    {
        const char* prev = at - 1;
        while (prev > obj->TextSrc + line_start_idx && (*prev & 0xC0) == 0x80)
            prev--;                                 // step back over a UTF-8 tail
        unsigned int c_prev;
        ImTextCharFromUtf8(&c_prev, prev, text_end);
        width += ImFontGetKerning(g.Font, g.FontSize, c_prev, c);
    }
    return width;
}
...
void ImGui::PushPasswordFont()
{
    ...
    IM_ASSERT(g.InputTextPasswordGlyph == NULL);
    ImFontGlyph* glyph = g.FontBaked->FindGlyphNoFallback(0x2022); // [Solace] Prefer the standard password bullet without remapping '*'.
    if (glyph == NULL)
        glyph = g.FontBaked->FindGlyph('*');

    // [Solace] A font's native bullet advance can be only a few pixels wide at
    // UI sizes, which makes a password look like one broken horizontal mark.
    // Give every mask a stable cell and center the glyph in it. The glyph is
    // restored in PopPasswordFont(), so ordinary U+2022 text keeps its metrics.
    g.InputTextPasswordGlyph = glyph;
    g.InputTextPasswordGlyphBackup = *glyph;
    const float mask_advance = ImMax(glyph->AdvanceX, ImFloor(g.FontSize * 0.40f + 0.5f));
    const float mask_offset = (mask_advance - glyph->AdvanceX) * 0.5f;
    glyph->X0 += mask_offset;
    glyph->X1 += mask_offset;
    glyph->AdvanceX = mask_advance;
...
void ImGui::PopPasswordFont()
{
    ...
    *g.InputTextPasswordGlyph = g.InputTextPasswordGlyphBackup;
    g.InputTextPasswordGlyph = NULL;
    IM_ASSERT(backup->IndexAdvanceX.Size == 0 && backup->IndexLookup.Size == 0);
}
